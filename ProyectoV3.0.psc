...
Funcion graficar
	...
	Mientras Que ( opcion >=1 )
	
FinFuncion

Funcion GraficarFuncionLineal
    Definir m, b, x, fx Como Real
    Definir i, j Como Entero
...
Funcion GraficarFuncionCuadratica
    Definir a, b, c, x, fx , R, X1,X2, Xv,Yv Como Real
    Definir i, j Como Entero
	Definir grafica Como Caracter
	Definir AX, AY Como Entero
	AX = 120
	...
	Si a > 0 Entonces
		Escribir "El Extremo corresponde a un Mínimo"
	SiNo
		Escribir "El Extremo corresponde a un Máximo"
	FinSi
	
	Xv = -b/(2*a)
	Xv = -Trunc((b*100)/(2*a))/100
	Yv = Trunc(( c -((b*b)/(4*a))*100)/100)
	Escribir "Coordenadas del vertice de la parábola (Xv; Yv): " Sin Saltar
	Escribir "(", Xv, ";", Yv, ") "
	
	R = b*b-4*a*c
	Si R<0 Entonces
		Escribir "La función cuadrática no corta el eje de abscisas, o sea no cuenta con raíces reales."
	SiNo
		X1= Trunc(((-b + Raiz(R))/2*a)*100)/100
		X2= Trunc (((-b - Raiz(R))/2*a)*100)/100
...
Funcion graficar_seno
	...
	AA = 18			// Amplitud de la función trigonométrica.
	Definir i, j,n Como Entero
	Definir AX, AY  Como Entero
	...
	Para j <- 1 Hasta AX+1 Hacer
		grafica[(AY/2)+1, j] <- "-" // A medio recorrido del eje y se grafica el Eje X con j entre [1-41]
	FinPara
	Repetir
...
Algoritmo matrices1
	...
	Escribir ""
	Escribir "                                                    Gracias por elegir SMART-MATRIX 2024"
	Escribir ""
	Escribir"------------------------------------------------------------------------------------------------------------------------------------------------"
FinAlgoritmo
