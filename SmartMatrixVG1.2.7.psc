...
Funcion GraficarFuncionCuadratica
    Definir a, b, c, x, fx, R, R1, X1, X2, Xv, Yv, X0 Como Real
    Definir i, j , Factor_escala Como Entero
	Definir grafica Como Caracter
	Definir AX, AY, aux Como Entero
	AX = 120
	AY = AX/3
	Factor_escala = 10
    Dimension grafica[AY+1, AX+1]  
	...
	Escribir "Coordenadas del vertice (Xv;Yv): " Sin Saltar
	Escribir "( ", Xv, " ; ", Yv, ") "
	...
	//tangente
	Repetir
		Escribir "¿Quiere representar la recta tangente en un punto?"
		Escribir "          (1) - Sí"
		Escribir "          (0) - No"
		Leer aux
		Si aux = 1 Entonces
			Borrar Pantalla
			Escribir "Ingrese el valor de X donde quiere observar la tangente"
			Leer X0
			tangente(X0, a, b, c, grafica, Factor_escala, Xv, Yv)
		FinSi
	Hasta Que aux = 0
	Escribir ""
	Escribir "Presione cualquier tecla para continuar"
...
Funcion tangente(X0 por valor, a por valor, b por valor, c por valor, grafica, Factor_escala por valor, Xv por valor, Yv por valor)
	Definir fderiv_enX0, f_enX0, fx, oo, x como Real
	Definir j , i, AX, AY como entero
	Definir grafica_tang Como Caracter
	AX = 120
	AY = AX/3
	a = a/Factor_escala
	b = b/Factor_escala
	...
	Si X0 = Xv Entonces
		fderiv_enX0 = 0
	SiNo
		fderiv_enX0 = 2*a*X0 + b
	FinSi
	
	f_enX0 = a*X0*X0 + b*X0 + c
	
	Dimension grafica_tang[AY+1, AX+1]  
	
	Para i <- 1 Hasta AY+1 Hacer
        Para j <- 1 Hasta AX+1 Hacer
            grafica_tang[i, j] = grafica[i, j]
        FinPara
    FinPara
	
	Para j = 1 Hasta AX+1 Hacer
        x = (j-(AX/2+1))
		fx = fderiv_enX0 * (x - x0) + f_enX0
        i = Redon(-fx+AY/2+1)
        Si i >= 1 y i <= AY Entonces
            grafica_tang[i, j] <- "+"
        FinSi
	...
	SiNo
		f_enX0 = a*X0*X0 + b*X0 + c
		fderiv_enX0 = 2*a*X0 + b
		oo = -X0*fderiv_enX0 + f_enX0
	FinSi

    // Imprimir la matriz
    Para i <- 1 Hasta AY+1 Hacer
        Para j <- 1 Hasta AX+1 Hacer
            Escribir Sin Saltar grafica_tang[i, j]
        FinPara
        Escribir ""
    FinPara
	
	Escribir ""
	Si fderiv_enX0 = 0 entonces
		Escribir "Ecuación de la recta tangente: T(x) = ", f_enX0
	SiNo
		Si oo >= 0 entonces
			Escribir "Ecuación de la recta tangente: T(x) = ", fderiv_enX0, "x + ", oo
		SiNo
			Escribir "Ecuación de la recta tangente: T(x) = ", fderiv_enX0, "x", oo
		FinSi
	FinSi
	Escribir "F(", X0, ") = ", f_enX0
	Escribir "Derivada de F(", X0, ") = ", fderiv_enX0
	Escribir ""
	Escribir "Presione cualquier tecla para continuar"
	Esperar Tecla
	Borrar Pantalla
...
Funcion graficar_seno
	Definir grafica Como Caracter
	Definir angulo, seno, escala , DEF , Amplitud, ordenada0 Como Real
	...
	Repetir
		...
		leer Amplitud
	Mientras Que Amplitud<=0 o Amplitud>100
	...
	FinMientras
	ordenada0 = sen(DEF*2*PI)
	ordenada0 = Trunc(ordenada0*100)/100
	// Imprimir la matriz
	Para i <- 1 Hasta AY+1 Hacer
		Para j <- 1 Hasta AX+1 Hacer
	...
	Escribir ""
	Escribir"                                      Valor de la ordenada al origen: ", ordenada0
	Escribir ""
...
Funcion GraficarFuncionLogaritmica
	...
	Escribir "  -------------------------------------------- FUNCIÓN LOGARITMO NATURAL ----------------------------------------------"
	Escribir ""
	...
	Para j <- 1 Hasta AX+1 Hacer
		...
		Si j>61 Entonces
			...
			fx<-10*LN((j-(61))/10) // Calcular Y en base a la ecuación 
			i<- Redon(-fx+AY/2+1)
		FinSi
...
Algoritmo SmartMatrix2024
	...
	Escribir "     "
	Escribir "		"
	Escribir "                         	   _____                              _       __  __           _                    	"
	Escribir "                         	  / ____|                            | |     |  \/  |         | |          (_)       	"
	Escribir "                         	 | (___    _ __ ___     __ _   _ __  | |_    | \  / |   __ _  | |_   _ __   _  __  __	"
	Escribir "                         	  \___ \  | ´_ ` _ \   / _` | | ´__| | __|   | |\/| |  / _` | | __| | ´__| | | \ \/ /	"
	Escribir "                         	  ____) | | | | | | | | (_| | | |    | |_    | |  | | | (_| | | |_  | |    | |  >  < 	"
	Escribir "                         	 |_____/  |_| |_| |_|  \__,_| |_|     \__|   |_|  |_|  \__,_|  \__| |_|    |_| /_/\_\	"
	Escribir "		"
	Escribir "		"
	Escribir "		"
	Escribir "		"
